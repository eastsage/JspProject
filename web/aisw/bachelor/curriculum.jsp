...
<head>
	...
	<link rel="stylesheet" type="text/css" href="../../_res/knu/aisw/css/top-of-curriculum.content350a.css" />
...
<div class="container">
	...
	<div class="container">
		...
		<div class="curriculum-top-box">
			<div>
				<p>좀 더 자세한 교육과정은 KNU 교육과정 사이트에 <br>상세하게 나와있습니다.</p>
				<a href="https://curriculum.kangwon.ac.kr/curriculum.jsp" target="_blank" title="새창열림"><span>KNU 교육과정 바로가기</span></a>
			</div>
		</div>
	</div>
  ...
  if (!(rs.next()))  {
   ...
   for(int j=startrow;j<=endrow;j++) {
    ...
    if (stepi > 0 ) {
     ...
     out.print("<A href=../community/freeboard/freeboard_read.jsp?id=");
...
//추가 끝
 ...
 if (wheregroup > 1) {
  out.println("[<A href=../community/freeboard/freeboard_list.jsp?gogroup=1>처음</A>]");
  out.println("[<A href=../community/freeboard/freeboard_list.jsp?gogroup="+priorgroup +">이전</A>]");
 } else {
  out.println("[처음]") ;
  out.println("[이전]") ;
 }
 if (name.size() !=0) {
  for(int jj=startpage; jj<=endpage; jj++) {
   if (jj==where)
    ...
    out.println("[<A href=../community/freeboard/freeboard_list.jsp?go="+jj+">" + jj + "</A>]") ;
   }
  }
  if (wheregroup < totalgroup) {
   out.println("[<A href=../community/freeboard/freeboard_list.jsp?gogroup="+ nextgroup+ ">다음</A>]");
   out.println("[<A href=../community/freeboard/freeboard_list.jsp?gogroup="+ totalgroup + ">마지막</A>]");
  } else {
   out.println("[다음]");
   out.println("[마지막]");
  }
 ...
 %>
			...
			<br><br>
			<FORM class="form-inline" method="post" name="msgsearch" action="../community/freeboard/freeboard_search.jsp">
				...
				<br>
				<div align=right><A href="../community/freeboard/freeboard_write.jsp"><img src="image/write.gif" class="img-fluid"></A></div>
			</FORM>
	</div>
