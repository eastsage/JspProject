...
//  Connection con= null;
 ...
 try {
  ...
  String sql = "select * from faq order by" ;
  ...
  if (!(rs.next()))  {
   ...
   for(int j=startrow;j<=endrow;j++) {
    ...
    if (stepi > 0 ) {
     ...
     out.print("<A href=faq_read.jsp?id=");
...
//추가 끝
 ...
 if (wheregroup > 1) {
  out.println("[<A href=faq_list.jsp?gogroup=1>처음</A>]");
  out.println("[<A href=faq_list.jsp?gogroup="+priorgroup +">이전</A>]");
 } else {
  out.println("[처음]") ;
  out.println("[이전]") ;
 }
 if (name.size() !=0) {
  for(int jj=startpage; jj<=endpage; jj++) {
   if (jj==where)
    ...
    out.println("[<A href=faq_list.jsp?go="+jj+">" + jj + "</A>]") ;
   }
  }
  if (wheregroup < totalgroup) {
   out.println("[<A href=faq_list.jsp?gogroup="+ nextgroup+ ">다음</A>]");
   out.println("[<A href=faq_list.jsp?gogroup="+ totalgroup + ">마지막</A>]");
  } else {
   out.println("[다음]");
   out.println("[마지막]");
  }
 ...
 %>
  ...
  <br><br>
  <FORM class="form-inline" method="post" name="msgsearch" action="faq_search.jsp">
   <div class="container_div">
    ...
    <br>
    <div align=right><A href="faq_write.jsp"><img src="image/write.gif" class="img-fluid"></A></div>
  </FORM>
 </div>
