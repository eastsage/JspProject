...
<div class="container">
 ...
 <FORM class="form-inline" method=post name=search action="faq_search.jsp">
  <div class="container">
   <div class="row form-group">
    [<A href="faq_list.jsp">자주하는질문으로 돌아가기</A>]
   </div>
   <div class="row form-group">
...
// Connection con= null;
 ...
 try {
  ...
  String sql = "select * from faq " + cond;
  ...
  if (!(rs.next()))  {
   ...
   for(int j=startrow;j<=endrow;j++) {
    ...
    String clink = "<A href=faq_read.jsp?id=" + keyid.elementAt(j) ;
...
//추가
    ...
    clink = "<div class=\"container_div\"><A href=faq_read.jsp?id=" + keyid.elementAt(j) ;
    clink = clink + "&page=" + where + ">" + subject.elementAt(j) + "</A>";
 ...
 if (wheregroup > 1) {
  out.print("[<A href=faq_search.jsp?gogroup=1");
  out.print("&stype="+ what+"&sval=" + val +">처음</A>]");
  out.print("[<A href=faq_search.jsp?gogroup="+priorgroup);
 ...
 if (name.size() !=0) {
  for(int jj=startpage; jj<=endpage; jj++) {
   ...
   out.print("[<A href=faq_search.jsp?go="+jj);
   out.print("&stype="+ what+"&sval=" + val +">" + jj + "</A>]") ;
   }
  }
 }
 if (wheregroup < totalgroup) {
  out.print("[<A href=faq_search.jsp?gogroup="+ nextgroup);
  out.print("&stype="+ what+"&sval=" + val +">다음</A>]");
  out.print("[<A href=faq_search.jsp?gogroup="+totalgroup);
